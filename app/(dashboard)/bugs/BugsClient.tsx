...
export default function BugsClient({ initialBugs }: { initialBugs: any[] }) {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    await addBug({ title, description });
            ...
            <form onSubmit={handleSubmit} className="space-y-4">
              <div>
                ...
                <input 
                  ...
                  placeholder="e.g. Postgres Connection Dropping" 
                />
              </div>
              <div>
                ...
                <textarea 
                  ...
                  className="w-full bg-[var(--bg-base)] border border-[var(--border)] rounded-md px-3 py-2 mt-1 text-[var(--text-primary)] focus:border-[var(--accent)] outline-none h-32 resize-none font-mono text-xs" 
              ...
              <div className="flex justify-end gap-3 pt-4 border-t border-[var(--border)]">
                ...
                <button type="submit" className="px-4 py-2 bg-red-500 text-white rounded-lg font-bold hover:brightness-110">Save Bug</button>
              </div>
            </form>
          </div>
        </div>
            ...
            {initialBugs.map((b) => (
              <div key={b.id} className="flex border-b border-[var(--border)] last:border-b-0 hover:bg-[var(--bg-surface)] transition-colors group">
                <div className="w-1/2 p-4 flex flex-col justify-center">
                  ...
                  {b.description && (
                    <div className="mt-1 ml-7 text-xs text-[var(--text-secondary)] line-clamp-1">{b.description}</div>
                  )}
                </div>
                <div className="w-1/4 p-4 flex items-center">
                  {b.status === "resolved" 
                    ...
                    : <button onClick={() => handleResolve(b.id)} className="flex items-center gap-1.5 text-xs text-red-500 border border-red-500/30 bg-red-500/10 px-2 py-1 rounded-md hover:bg-red-500 hover:text-white transition-colors cursor-pointer group-hover:visible"><CircleDashed className="w-3 h-3"/> Mark Resolved</button>
                  }
                </div>
