...
export default function FlashcardsClient({ initialCards }: { initialCards: any[] }) {
  ...
  const handleScore = async (change: number) => {
    ...
    if (card) {
      await updateFlashcardScore(card.id, change);
    }
    setFlipped(false);
    setTimeout(() => {
          ...
          <div className="bg-[var(--bg-elevated)] border border-[var(--border)] p-6 rounded-xl w-full max-w-lg">
            ...
            <form onSubmit={handleSubmit} className="space-y-4">
              ...
              <div className="flex justify-end gap-3 pt-4 border-t border-[var(--border)]">
                ...
                <button type="submit" className="px-4 py-2 bg-[var(--accent)] text-black rounded-lg font-bold hover:brightness-110">Save Card</button>
              </div>
            </form>
          </div>
        </div>
      ...
      {initialCards.length === 0 ? (
        ...
        <div className="flex-1 flex flex-col items-center justify-center -mt-12">
          ...
          <div className="relative w-full max-w-2xl min-h-[320px] perspective-1000 cursor-pointer" onClick={() => setFlipped(!flipped)}>
            <div className={`absolute inset-0 w-full h-full transition-transform duration-500 transform-style-3d ${flipped ? 'rotate-y-180' : ''}`}>
              ...
              <div className={`absolute w-full h-full bg-[var(--bg-elevated)] border-2 border-[var(--accent)] shadow-[0_0_30px_rgba(0,255,170,0.1)] rounded-2xl p-10 flex flex-col items-center justify-center backface-hidden rotate-y-180 transition-opacity ${!flipped ? 'opacity-0' : 'opacity-100'}`}>
                ...
                <p className="text-xl text-center text-[var(--text-primary)] leading-relaxed">{initialCards[currentIndex].answer}</p>
              </div>
          ...
          <div className={`mt-12 flex gap-4 transition-opacity duration-300 ${flipped ? 'opacity-100' : 'opacity-0 pointer-events-none'}`}>
            <button onClick={() => handleScore(-1)} className="flex items-center gap-2 px-6 py-3 rounded-xl bg-[var(--bg-surface)] border border-[var(--border)] text-red-500 hover:bg-red-500/10 hover:border-red-500/30 transition-all font-bold shadow-sm">
              <ThumbsDown className="w-5 h-5" /> Forgot
            </button>
            <button onClick={() => handleScore(1)} className="flex items-center gap-2 px-6 py-3 rounded-xl bg-[var(--accent)] text-black border border-transparent shadow-[0_4px_14px_0_rgba(0,255,170,0.39)] hover:shadow-[0_6px_20px_rgba(0,255,170,0.23)] hover:brightness-110 transition-all font-bold">
